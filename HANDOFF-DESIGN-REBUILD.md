...
1. ~~customer-portal.html~~      ← ✅ FULLY DONE. Overview tab rebuilt 2026-04-24 02:50 CT (commit 19e2d76, approved). Other 8 tabs (Estimates/Projects/Invoices/Maintenance/Referrals/Colors/Documents/Settings) verified on-spec 2026-04-26 — token inheritance from the Overview rebuild carried through cleanly across all 8 (orange spine, hairline cards, Sora headlines, status pills, orange section dots, no residual gold/cream, no Space Grotesk, no gradients). No additional rebuild needed.
...
9. ~~work-order-crew.html~~  ← ✅ ALREADY DONE in commit 64acdea ("Add Mark-Finished FAB to work-order-crew.html"). Found during 2026-04-26 verification: Mark-Finished FAB (orange checkmark, 56×56, 4px radius, 88px right offset to sit left of Daily Update), bottom-sheet confirmation panel with the "telling office you're done" warning + "the standard" yellow callout + 4-item walk-the-job checklist, Not yet / Yes — I'm finished buttons, localStorage persistence (key `nashos.wo-crew.markedFinished.WO-50281947`), toast "Marked finished · office has been notified", FAB turns navy when in .done state. Page is full-EI gold standard, no design rebuild was required.
...
13. ~~campaign-builder.html~~ ← ✅ REBUILT 2026-04-26 (commit 86b1214). Internal lifecycle/drip orchestration desktop tool; pairs with direct-mail.html (Campaign Builder owns timing/sequencing/audience, Direct Mail owns fulfillment). Top-level tab strip with 2px orange underline on active, navy/orange/green/muted day-marker circles, step-card expanded state with navy outline + 3px orange left rule, suppression sidebar with orange toggles, merge tags as surface tiles with orange hover.
